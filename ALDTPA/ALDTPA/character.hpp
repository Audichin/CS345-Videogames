...
public:
    ...
    virtual void update(float dt){
        ...
        while (totalMillis>frameTime) {
            which++;
            if (which>=static_cast<int>(textures.size())) which=0;
            totalMillis-=frameTime;
        }
    }
    void draw(SDL_Renderer *renderer,int offsetx=0,int offsety=0){
        if (dead) return;
        if (textures.empty() || textures[which]==NULL) throw "About to render a null texture";
...
public:
    ...
    void keyEvent(SDL_Keycode symbol){
        ...
        if (symbol==SDLK_a) px-=32.0f;
        if (symbol==SDLK_d) px+=32.0f;
        if (symbol==SDLK_w) py-=32.0f;
        if (symbol==SDLK_s) py+=32.0f;
        moveTo(static_cast<int>(px), static_cast<int>(py));
    } 
